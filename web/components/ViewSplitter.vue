...
import { ref, onMounted, onUnmounted, useSlots, watch, nextTick } from 'vue'
...
const updatePanes = async () => {
  ...
  const children = Array.from(containerRef.value.children).filter(
    (el): el is HTMLElement =>
      ...
      !el.classList.contains('splitter-handle')
  )
  ...
  if (
    paneSizes.value.length !== visiblePaneCount ||
    paneSizes.value.some((s) => s === 0)
  ) {
...
const addSplitterHandles = (panes: HTMLElement[]) => {
  ...
  containerRef.value
    ?.querySelectorAll('.splitter-handle')
