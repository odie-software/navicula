  ...
  <q-layout view="lHh Lpr lFf" class="bg-white">
    ...
    >
       <q-list v-if="!pending && navigationItems.length > 0" padding>
         <template v-for="item in navigationItems" :key="item.id">
           ...
           <q-item
             ...
             class="nav-item"
             @mouseover="hoveredItemId = item.id" @mouseleave="hoveredItemId = null"
           >
             ...
             <q-item-section side top class="q-mini-drawer-hide split-button-container">
                <q-btn
                    v-show="hoveredItemId === item.id && !displayedAppIds.includes(item.id) && displayedAppIds.length > 0"
                    flat dense round
                    icon="add_box"
                    size="md"
                    @click.stop="addWindow(item)"
                >
                  <q-tooltip anchor="center right" self="center left">Add to view</q-tooltip>
                </q-btn>
             </q-item-section>
           </q-item>
           ...
           >
             ...
             <q-list dense padding class="q-pl-lg">
               <q-item
                 ...
                 dense
                 class="nav-item"
                 @mouseover="hoveredItemId = app.id" @mouseleave="hoveredItemId = null"
               >
                 <q-item-section avatar>
                   <q-icon :name="app.icon" size="xs"/>
                 </q-item-section>
                 <q-item-section class="q-mini-drawer-hide">
                   <q-item-label class="text-body2">{{ app.title }}</q-item-label>
                 </q-item-section>
                 <!-- Add Button (Nested) -->
                 <q-item-section side top class="q-mini-drawer-hide split-button-container">
                    <q-btn
                        v-show="hoveredItemId === app.id && !displayedAppIds.includes(app.id) && displayedAppIds.length > 0"
                        flat dense round
                        icon="add_box"
                        size="xs"
       ...
       <q-item v-else-if="pending">
        ...
        <q-item v-else-if="error">
          <q-item-section class="text-negative q-pa-md q-mini-drawer-hide">Error loading configuration.</q-item-section>
       </q-item>
       <q-item v-else>
          <q-item-section class="q-pa-md q-mini-drawer-hide">No navigation items found.</q-item-section>
       </q-item>
       ...
       <q-item v-if="userInfo.userEmail" class="q-mini-drawer-hide" dense>
          ...
          <q-item-section>
             <q-item-label class="text-caption ellipsis">{{ userInfo.userEmail }}</q-item-label>
             <q-item-label caption>Role: {{ userInfo.role }}</q-item-label>
          </q-item-section>
       </q-item>

       <!-- Dark Mode Toggle -->
       <q-item clickable @click="$q.dark.toggle()" dense>
         <q-item-section avatar>
           <q-icon :name="$q.dark.isActive ? 'dark_mode' : 'light_mode'" size="sm"/>
         </q-item-section>
         <q-item-section class="q-mini-drawer-hide">
       ...
       <!-- Drawer Mode Toggle -->
       <q-item clickable @click="toggleDrawerMode" dense>
         <q-item-section avatar>
           <q-icon :name="drawerMode === 'always-open' ? 'keyboard_arrow_left' : 'keyboard_arrow_right'" size="sm"/>
         </q-item-section>
         <q-item-section class="q-mini-drawer-hide">
           <q-item-label>{{ drawerMode === 'always-open' ? 'Auto Hide' : 'Keep Open' }}</q-item-label>
         </q-item-section>
         <q-tooltip anchor="center right" self="center left">{{ drawerMode === 'always-open' ? 'Switch to auto-hide mode' : 'Keep drawer always open' }}</q-tooltip>
       </q-item>

       <!-- Version Display -->
       <div class="absolute-bottom q-pa-sm text-center text-caption text-grey-7 q-mini-drawer-hide">
         Version: {{ appVersion }}
       </div>
     </q-drawer>
    ...
    <q-page-container>
       <q-page class="page-flex-container">
         <!-- Single Window View -->
         <div v-if="displayedAppIds.length === 1" class="app-window-container single-window">
           <keep-alive>
             <AppWindow
               v-if="getAppLinkById(displayedAppIds[0])"
               :ref="(el: any) => { if (el) appWindowRefs[displayedAppIds[0]] = el as InstanceType<typeof AppWindow> }"
               :key="displayedAppIds[0]"
              :app-link="getAppLinkById(displayedAppIds[0])!"
              :is-active="true"
              :is-same-origin="isAppLinkSameOrigin(getAppLinkById(displayedAppIds[0]))"
            ...
            <template v-slot:after>
              <keep-alive>
                 <AppWindow
                      ...
                      :ref="(el: any) => { if (el) appWindowRefs[displayedAppIds[1]] = el as InstanceType<typeof AppWindow> }"
                      :key="displayedAppIds[1]"
                      :app-link="getAppLinkById(displayedAppIds[1])!"
                      :is-active="activeAppId === displayedAppIds[1]"
                      :is-same-origin="isAppLinkSameOrigin(getAppLinkById(displayedAppIds[1]))"
                      :default-toolbar-color="defaultToolbarColor"
                      @activate-window="activateWindow"
                      @close-window="closeWindow"
         ...
         <div v-else-if="displayedAppIds.length === 0 && !pending" class="flex flex-center text-h6 text-grey-7" style="height: 100%;">
           {{ navigationItems.length > 0 ? 'Select an item from the menu' : 'No items configured' }}
         </div>
...
function toggleDrawerMode() {
  drawerMode.value = drawerMode.value === 'auto-hide' ? 'always-open' : 'auto-hide';
...
function selectItem(item: AppLink | null) {
    ...
    if (displayedAppIds.value.includes(item.id)) {
        ...
        const activeIndex = activeAppId.value ? displayedAppIds.value.indexOf(activeAppId.value) : -1;
...
// openInSplitView is now addWindow
const openInSplitView = addWindow;
...
function closeWindow(appIdToClose: string) {
    ...
    if (index > -1) {
        ...
        if (activeAppId.value === appIdToClose) {
            const newActiveId = previousActiveAppId.value && displayedAppIds.value.includes(previousActiveAppId.value)
...
function reloadIframe(appId: string) {
  ...
  if (iframe) {
      iframe.src = iframe.src; // Standard way to reload an iframe
      $q.notify({ type: 'info', message: `Reloading ${app?.title}...` });
  } else {
      $q.notify({ type: 'warning', message: `Could not find iframe to reload ${app?.title}.` });
  }
}
...
async function copyIframeUrl(appId: string) {
  ...
  if (!iframe?.contentWindow || !isAppLinkSameOrigin(appLink)) {
     $q.notify({ type: 'negative', message: 'Cannot access iframe URL (cross-origin or not loaded).' });
    return;
  }
  try {
...
function handleKeydown(event: KeyboardEvent) {
    if (event.altKey && event.key === 'Tab') {
        ...
        } else if (previousActiveAppId.value && displayedAppIds.value.includes(previousActiveAppId.value)) {
    ...
    if (event.altKey && (event.key === 'ArrowUp' || event.key === 'ArrowDown')) {
        ...
        const currentActiveIndex = activeAppId.value ? links.findIndex(link => link.id === activeAppId.value) : -1;
        let nextIndex = -1;

        if (event.key === 'ArrowDown') {
            nextIndex = currentActiveIndex >= links.length - 1 ? 0 : currentActiveIndex + 1;
        } else {
            nextIndex = currentActiveIndex <= 0 ? links.length - 1 : currentActiveIndex - 1;
        }
    ...
    if (targetAppId) {
        ...
        if (targetApp) {
            ...
            console.warn(`Keybinding "${keyString}" targets unknown app ID "${targetAppId}"`);
        }
    }
}
...
const { data: configData, pending, error } = useFetch<ConfigResponse>('/api/config');
...
watch(configData, (newConfig) => {
  if (newConfig && !newConfig.error && Array.isArray(newConfig.navigationItems)) {
    ...
    const validAppIdsFromQuery = appsFromQuery.filter(id => allAppLinks.value.some(app => app.id === id));
    const validActiveIdFromQuery = validAppIdsFromQuery.includes(activeFromQuery ?? '') ? activeFromQuery : null;
    ...
    if (displayedAppIds.value.length > 1 && activeAppId.value) {
        previousActiveAppId.value = displayedAppIds.value.find(id => id !== activeAppId.value) || null;
    } else {
        previousActiveAppId.value = null;
    }
    ...
    console.error("Failed to load or parse configuration:", newConfig?.error || error.value || "Invalid config format");
...
watch(() => route.query, (newQuery) => {
    ...
    if (JSON.stringify(stateAppsSorted) !== JSON.stringify(queryAppsSorted)) {
         ...
         activeAppId.value = validActiveIdFromQuery || displayedAppIds.value[0] || null;
