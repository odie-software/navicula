...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!config || typeof config !== 'object' || !Array.isArray(config.navigationItems)) {
        throw new Error("Invalid config format: 'navigationItems' array is missing or invalid.");
    }
     if (!config.roles || !config.acl || !config.defaultToolbarColor) {
         console.warn("Config warning: 'roles', 'acl', or 'defaultToolbarColor' might be missing.");
     }

  } catch (error) {
    console.error(`Error reading or parsing config file at ${configPath}:`, error);
    setResponseStatus(event, 500);
    return { error: 'Failed to load server configuration', userEmail: null, role: DEFAULT_ROLE, navigationItems: [], defaultToolbarColor: 'primary', keybindings: {} };
  }
  ...
  if (config.useRemoteAuth) {
      const userEmailHeader = (getHeader(event, 'remote-user') || getHeader(event, 'x-forwarded-user') || '').toLowerCase();
      userEmail = userEmailHeader || null;
      userRoleName = userEmail ? (config.acl[userEmail] || DEFAULT_ROLE) : DEFAULT_ROLE;
  ...
  if (!userRole) {
      console.error(`Default role "${DEFAULT_ROLE}" or assigned role "${userRoleName}" not found in config`);
      setResponseStatus(event, 500);
      return { error: 'Server configuration error: Role definition missing', userEmail, role: DEFAULT_ROLE, navigationItems: [], defaultToolbarColor: config.defaultToolbarColor || 'primary', keybindings: config.keybindings || {} };
  }
  ...
  if (config.navigationItems) {
    config.navigationItems.forEach(item => {
      if (isAppLink(item)) {
        ...
        const accessibleApps = item.apps.filter(app =>
